...
class GameCore():
    ...
    获取候选的动作
    """
    def _obtain_candidate_action(self, env, last_player_role, last_primary_item, last_action):
        action_reward = list()
        for action in ActionTypeEnum.ALL_ACTION:
            _, score, done, info = env.step(action, last_player_role, last_primary_item, last_action)
            error = info['error']
            if not error:
                if done:
                    self._game_is_over = True
                action_reward.append((action, score))
                break
            env.restore()
    ...
    """
    def obtain_put_card(self, player, surround, last_player_role, last_card_type_struct=None, last_action=None):
        env = Env()
        env.specify_env(None, None)
        action_reward = self._obtain_candidate_action(env, last_player_role, last_card_type_struct, last_action)
        ...
        return accpet_action, put_card, primary_item, obser

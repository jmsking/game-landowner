...
class KickerNet():
    def __init__(self, x_dim=15, y_dim=4, z_dim=3, batch_size = 5,
                n_action=config.N_KICKER_ACTION, n_display=5, 
    ...
    def start_train(self, input_x, input_y):
        with tf.Session() as sess:
            ...
            saver.save(sess, config.KICKER_MODEL_SAVE_PATH)
            print('Model Save Success.')
    
    def predict(self, input_data):
        with tf.Session() as sess:
            saver = tf.train.import_meta_graph(config.KICKER_META_SAVE_PATH)
            model_file = tf.train.latest_checkpoint(config.KICKER_MODEL_PARENT_PATH)
...
if __name__ == "__main__":
    train_inputs, train_output = PreprocUtils.gen_kicker_net_input()

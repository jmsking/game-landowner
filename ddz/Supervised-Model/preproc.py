...
class PreprocUtils(object):
    ...
    Returns:
      train_inputs: input of policy network
      train_output: output of policy network
    """
    @staticmethod
    def _gen_policy_net_input_core(samples):
        ...
        for sample in samples:
            ...
            for item in sample[:-1]:
                ...
                role = int(role)
                _, _, card_dict = PreprocUtils._decode_put_card(put_card)
                for card in card_dict:
                    num = card_dict[card]
                    one_input[card-3][num-1][role] = 1
            train_inputs.append(one_input)
            role, put_card = sample[-1].split(",")
            p_action, _, _ = PreprocUtils._decode_put_card(put_card)
            train_output.append([p_action])
        return np.array(train_inputs), np.array(train_output)

    """
    Generate the input of kicker network
    Args:
    samples: generated by method <gen_state_action_pair>
    Returns:
      train_inputs: input of kicker network
      train_output: output of kicker network
    """
    @staticmethod
    def _gen_kicker_net_input_core(samples):
        train_inputs = list()
        train_output = list()
        for sample in samples:
            one_input = np.zeros([15,4,3])
    ...
    def _decode_put_card(put_card):
        ...
        if put_card != "PASS":
            p_action, k_action, card_dict = HandCardUtils.decode_put_card(put_card)

...
class DeepQNet:
    ...
    def update_weight(self, optimizer, tvars, name='value'):
        """
        ...
        for idx in range(len(tvars)):
            grad_op.append(tf.placeholder(tf.float32, name=f'{name}_grad_{idx}'))
        update_grad = optimizer.apply_gradients(zip(grad_op, tvars))
        return grad_op, update_grad

    def __save_model(self, sess, input_op, net_out):
        builder = tf.compat.v1.saved_model.builder.SavedModelBuilder(self.model_path)
        inputs = {TF_CONST['INPUT_KEY']: tf.compat.v1.saved_model.utils.build_tensor_info(input_op)}
        outputs = {TF_CONST['OUTPUT_KEY']: tf.compat.v1.saved_model.utils.build_tensor_info(net_out)}
        signature = tf.compat.v1.saved_model.build_signature_def(inputs, outputs, TF_CONST['METHOD_NAME'])
    ...
    def train(self):
        ...
        with self.graph.as_default():
            ...
            value_grad_op, update_value_grad = self.update_weight(optimizer, tvars[:mid])
            target_grad_op, update_target_grad = self.update_weight(optimizer, tvars[mid:], name='target')
            self.init_variables()
            ...
            for epoch in range(1, self.n_epoch+1, 1):
                ...
                feed_dict = dict(zip(value_grad_op, grad))
                self.sess.run(update_value_grad, feed_dict=feed_dict)
                # update target network parameters
                if epoch % self.n_update == 0:
                    sum_grad = np.sum(grad_collection, axis=0)
                    feed_dict = dict(zip(target_grad_op, sum_grad))
                    self.sess.run(update_target_grad, feed_dict=feed_dict)

...
class ProcessFourDouStrategy(BaseProcessAction):
    """
    E.g. [33334455], [44447788], [55554499], ......
    """
    ...
    def run(self):
        ...
        if len(remain_card) < 2:
            logger.debug('Can not accept the card')
            return None, None, None
        other_card = sorted(random.sample(remain_card, 2))
        score = HandCardUtils.value_map(one_card, CardTypeEnum.CT_FOUR_DOU, 8)
        put_card = [one_card]*4

...
class UNet(object):
    ...
    def build_net(self):
        self.input_x = tf.placeholder(tf.float32, [None, self.n_input+self.n_action*3], name='input_x')
        #self.W1 = tf.Variable(tf.random_normal([self.n_input, self.n_hidden],mean=0,stddev=1),name='W1')
        self.W1 = tf.get_variable('W1', shape=[self.n_input+self.n_action*3, self.n_hidden], 
        ...
        self.auxi_input_x = tf.placeholder(tf.float32, [None, self.n_input+self.n_action*3], name='auxi_input_x')
        self.AW1 = tf.get_variable('AW1', shape=[self.n_input+self.n_action*3, self.n_hidden], 
                                    initializer=tf.contrib.layers.xavier_initializer())
    ...
    def gen_mini_batch(self, sess, buffer_X, buffer_R, buffer_AX):
        ...
        while True:
            ...
            rs_o.append(reward)
            if done:
                if len(xs_o) >= self.window and len(xs_l) >= self.window and len(xs_u) >= self.window:
    ...
    def _obtain_reward(self, r1, r2, r3, is_lost_round):
        #print(r1, r2, r3)
        aux_score = 0
        if is_lost_round:
            aux_score -= 0.5
            ...
            aux_score += 0.5
        reward = 1/(r1+1) + r2 + 1/(r3+1) + aux_score
        #print(reward)
        return reward

    def _obtain_joint_action(self, v1, v2, v3):
        act1 = [0] * self.n_action
        act1[v1] = 1
        act2 = [0] * self.n_action
        act2[v2] = 1
        act3 = [0] * self.n_action
        act3[v3] = 1
        return act1 + act2 + act3

    def obtain_sample(self, sess, xs_o, vs_o, rs_o, xs_l, vs_l, rs_l, xs_u, vs_u, rs_u):
        del xs_o[0]
        del vs_o[0]
        del rs_o[0]
        del xs_l[0]
        ...
        for ix in range(len(xs_u)-1):
            is_lost_round = (xs_u[ix+1] == ActionTypeEnum.ACTION_NO_PUT.value)
            next_x = xs_u[ix+1]
            curr_x = xs_u[ix]
            curr_r = self._obtain_reward(rs_u[ix], rs_o[ix], rs_l[ix], is_lost_round)
            curr_act = self._obtain_joint_action(vs_u[ix], vs_o[ix], vs_l[ix])
            max_reward = -1e10
            auxi_input_x = None
            for action in range(self.n_action):
                ...
                tmp_x = np.reshape(tmp_x, [1, self.n_input+self.n_action*3])
    ...
    def start_train(self):
        with tf.Session() as sess:
            ...
            writer = tf.summary.FileWriter(config.U_LOG_SAVE_PATH, sess.graph)
            ...
            while t <= self.n_epoch:
                ...
                print('[U-Net] cost for epoch %d : %f.' %(t, cost))
                ...
                #self.update_auxi_net_var(tvars, sess)
                t += 1
            saver.save(sess, config.U_NET_MODEL_SAVE_PATH)
            print('--------[U-Net] training successful--------')
    ...
    def predict(self):
        ...
        with tf.Session() as sess:
            saver = tf.train.import_meta_graph(config.U_NET_MODEL_META_PATH)
            model_file=tf.train.latest_checkpoint(config.U_NET_MODEL_PATH)
            ...
            while True:
                ...
                for action in range(self.n_action-1):
                    ...
                    x.extend(act)
                    x = np.reshape(x, [1, self.n_input+self.n_action*3])
...
if __name__ == '__main__':
    agent = UNet(n_input=config.N_INPUT, n_output=config.N_OUTPUT)
    agent.start_train()
    #agent.predict()
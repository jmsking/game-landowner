...
class ONet(object):
    ...
    def obtain_sample(self, sess, xs_o, vs_o, rs_o, xs_l, vs_l, rs_l, xs_u, vs_u, rs_u):
        ...
        for ix in range(len(xs_o)-1):
            is_lost_round = (xs_o[ix+1] == ActionTypeEnum.ACTION_NO_PUT.value)
            next_x = xs_o[ix+1]
            curr_x = xs_o[ix]
            curr_r = self._obtain_reward(rs_o[ix], rs_l[ix], rs_u[ix], is_lost_round)
            curr_act = self._obtain_joint_action(vs_o[ix], vs_l[ix], vs_u[ix])
            max_reward = -1e10
            auxi_input_x = None
    ...
    def start_train(self):
        with tf.Session() as sess:
            ...
            writer = tf.summary.FileWriter(config.O_LOG_SAVE_PATH, sess.graph)
            ...
            while t <= self.n_epoch:
                ...
                print('[O-Net] cost for epoch %d : %f.' %(t, cost))
                ...
                #self.update_auxi_net_var(tvars, sess)
                t += 1
            saver.save(sess, config.O_NET_MODEL_SAVE_PATH)
            print('--------[O-Net] training successful--------')
    ...
    def predict(self):
        ...
        with tf.Session() as sess:
            saver = tf.train.import_meta_graph(config.O_NET_MODEL_META_PATH)
            model_file=tf.train.latest_checkpoint(config.O_NET_MODEL_PATH)
...
if __name__ == '__main__':
    agent = ONet(n_input=config.N_INPUT, n_output=config.N_OUTPUT)
    agent.start_train()
    #agent.predict()
...
class LNet(object):
    ...
    def _obtain_reward(self, r1, r2, r3, is_lost_round):
        ...
        if is_lost_round:
            ...
            aux_score += 0.5
        reward = 1/(r1+1) + 1/(r2+1) + r3 + aux_score
        #print(reward)
        return reward
    ...
    def obtain_sample(self, sess, xs_o, vs_o, rs_o, xs_l, vs_l, rs_l, xs_u, vs_u, rs_u):
        ...
        for ix in range(len(xs_l)-1):
            is_lost_round = (xs_l[ix+1] == ActionTypeEnum.ACTION_NO_PUT.value)
            next_x = xs_l[ix+1]
            curr_x = xs_l[ix]
            curr_r = self._obtain_reward(rs_l[ix], rs_u[ix], rs_o[ix], is_lost_round)
            curr_act = self._obtain_joint_action(vs_l[ix], vs_u[ix], vs_o[ix])
            max_reward = -1e10
            auxi_input_x = None
    ...
    def start_train(self):
        with tf.Session() as sess:
            ...
            writer = tf.summary.FileWriter(config.L_LOG_SAVE_PATH, sess.graph)
            ...
            while t <= self.n_epoch:
                ...
                print('[L-Net] cost for epoch %d : %f.' %(t, cost))
                ...
                #self.update_auxi_net_var(tvars, sess)
                t += 1
            saver.save(sess, config.L_NET_MODEL_SAVE_PATH)
            print('--------[L-Net] training successful--------')
    ...
    def predict(self):
        ...
        with tf.Session() as sess:
            saver = tf.train.import_meta_graph(config.L_NET_MODEL_META_PATH)
            model_file=tf.train.latest_checkpoint(config.L_NET_MODEL_PATH)
...
if __name__ == '__main__':
    agent = LNet(n_input=config.N_INPUT, n_output=config.N_OUTPUT)
    agent.start_train()
    #agent.predict()